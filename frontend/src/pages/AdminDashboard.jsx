...
const AdminDashboard = () => {
  ...
  const rejectAmbulance = (id) => {
    setAmbulances((prev) =>
      prev.map((amb) =>
        amb.id === id ? { ...amb, status: 'Rejected' } : amb
      )
    );
    setSelected(null);
  };
...
const contentWrap = {
  position: 'relative',
  zIndex: 2,
  height: '85vh', // Take up remaining space
};
...
const grid = {
  ...
  height: '100%', // Fill the content wrap
};
...
const panelLeft = {
  ...panel,
  maxHeight: '75vh', // Constrain the sidebar height
};

const panelRight = {
  ...panel,
  maxHeight: '75vh', // Constrain the map panel height
};

const listScroll = {
  overflowY: 'auto', // ✅ Independent scroll for the ambulance list
  flex: 1,
  paddingRight: '5px',
};
